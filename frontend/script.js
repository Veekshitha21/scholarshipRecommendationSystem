...
async function init() {
  ...
  if (loaded.length) {
    ...
    setStatus(`Loaded ${loaded.length} scholarships. Ready to find your matches!`, "ok");
  } else {
    setStatus("Using built-in scholarship database", "info");
  }
  ...
  profileForm.addEventListener("submit", (e) => {
    ...
    (async () => {
      ...
      if (!ok) {
        setStatus('Using local AI matching...', 'info');
        runSmartMatch();
      }
    })();
  });
...
function renderRegistered() {
  ...
  if (!state.registered.length) {
    emptyRegistered.style.display = "block";
    registeredCount.textContent = "0 saved";
    return;
  }

  emptyRegistered.style.display = "none";
...
function makeCard(scholarship, allowRegister) {
  ...
  scoreChip.textContent = `${Math.round(scholarship.score)}% Match`;
  
  // Color code the score chip
  ...
  node.querySelector(".meta").textContent = `${displayEducation(scholarship.education_level)} • ${scholarship.gender || "Any"}`;
  ...
  btn.addEventListener("click", () => {
    ...
    if (!exists) {
      ...
      renderRegistered();
      renderRecommendations(); // Re-render to show updated status
    }
